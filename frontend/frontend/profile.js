...
function populateTimezoneSelect(selectedValue) {
  ...
  if (options.includes(preferred)) {
    els.reminderTimezone.value = preferred;
  } else {
    els.reminderTimezone.value = 'UTC';
  }
}
...
function scheduleLocalReminder(prefs) {
  ...
  if (!prefs.dailyReminder || !prefs.push || !('Notification' in window) || Notification.permission !== 'granted') {
    return;
  }
  ...
  notificationTimer = setTimeout(() => {
    new Notification('Atlas wellness check-in', {
      body: 'Take a minute to check in with your sleep, stress, energy, and hydration.',
    });
...
async function loadNotifications() {
  ...
  try {
    ...
    els.dailyReminder.checked = prefs.dailyReminder !== false;
    els.reminderTime.value = prefs.reminderTime || '20:00';
    els.weeklySummary.checked = prefs.weeklySummary !== false;
    els.pushNotifications.checked = !!prefs.push;
    els.emailNotifications.checked = prefs.email !== false;
    scheduleLocalReminder(prefs);
  } catch (error) {
...
if (els.notificationsForm) els.notificationsForm.addEventListener('submit', async (e) => {
  ...
  const prefs = {
    dailyReminder: els.dailyReminder.checked,
  ...
  try {
    ...
    scheduleLocalReminder(data.preferences || prefs);
